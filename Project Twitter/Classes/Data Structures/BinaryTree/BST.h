...
template <class T>
List<T>* AVLTree<T>::toList(List<T>* list, BTNode<T>* node){
	if (node->getLeft() != NULL){
		list = toList(list, node->getLeft());
	}
	list->addFront(node->getData());
	if (node->getRight() != NULL){
		list = toList(list, node->getRight());
	}
	return list;
}

template <class T>
List<T> AVLTree<T>::toList(){
	List<T>* list = new List<T>;
	List<T> temp = *toList(list, head);
	delete list;
	return temp;
}
